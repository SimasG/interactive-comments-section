...
async function displayData() {
  ...
  for (let i = 0; i < data.comments.length; i++) {
    if (data.comments[i].replies.length > 0) {
      ...
      commentReplies.forEach((commentReply) => {
        ...
        main.insertAdjacentHTML("afterbegin", replyHtml);
      });
    }
  }
  ...
  for (let comment of data.comments) {
    ...
    //     </div>
    //   </section>`;
    // main.insertAdjacentHTML("afterbegin", commentHtml);
  }
...
// having trouble accessing html elements from the template string literals
const btnSubmit = document.querySelector(".btn-submit-comment");

btnSubmit.addEventListener("click", createComment());
...
async function createComment() {
  ...
  let comment = {
    content: txt.value,
    createdAt: Date.now(),
    currentUser: data.currentUser.image.png,
  };
  //   addComment(comment);
